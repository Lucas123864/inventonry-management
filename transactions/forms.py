...
class SelectSupplierForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['discount'].widget.attrs.update({'class': 'textinput form-control','maxlength': '2', 'pattern' : '[0-9]{2}', 'title' : 'Numbers only'})
        self.fields['vat'].widget.attrs.update({'class': 'textinput form-control','maxlength': '2', 'pattern' : '[0-9]{2}', 'title' : 'Numbers only'})
        self.fields['delivery_fee'].widget.attrs.update({'class': 'textinput form-control', 'pattern' : '[0-9]{5}', 'title' : 'Numbers only'})
        self.fields['vehicle'].widget.attrs.update({'class': 'textinput form-control',})
        self.fields['eta'].widget.attrs.update({'class': 'textinput form-control'})
    class Meta:
        model = PurchaseBill
        fields = ['supplier','discount', 'vat', 'delivery_fee', 'eta', 'vehicle']
...
class SaleForm(forms.ModelForm):
    ...
    class Meta:
        model = SaleBill
        fields = ['name', 'phone', 'address', 'email', 'discount', 'vat', 'delivery_fee', 'eta', 'vehicle']
        widgets = {
            'address' : forms.Textarea(
                attrs = {
                    'class' : 'textinput form-control',
